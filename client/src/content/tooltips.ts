...
export const auditScheduleTooltips: Record<string, TooltipContent> = {
  schedule: { text: 'Run audits automatically on a recurring schedule to catch regressions.', guideLink: '/guide/seo-speed-aeo/scheduling' },
  frequency: { text: 'How often the audit runs. Daily catches issues fast; weekly is less noisy.', guideLink: '/guide/seo-speed-aeo/scheduling#frequency' },
  enabled: { text: 'Pause or resume the schedule without deleting it.', guideLink: '/guide/seo-speed-aeo/scheduling#controls' },
};

// ─── Code Review ────────────────────────────────────────────────

export const codeReviewTooltips: Record<string, TooltipContent> = {
  codeReview: { text: 'AI checks custom embeds for performance, security, and Webflow compatibility.', guideLink: '/guide/seo-speed-aeo/code-review' },
  codeType: { text: 'Select the language of your embed code for accurate analysis.', guideLink: '/guide/seo-speed-aeo/code-review#types' },
};

// ─── Page Transitions ───────────────────────────────────────────

export const pageTransitionTooltips: Record<string, TooltipContent> = {
  pageTransition: { text: 'CSS animations that play when navigating between pages.', guideLink: '/guide/animations/page-transitions' },
  viewTransitions: { text: 'Uses the View Transitions API for smooth cross-page animations.', guideLink: '/guide/animations/page-transitions#view-transitions' },
};

// ─── Performance ────────────────────────────────────────────────

export const performanceTooltips: Record<string, TooltipContent> = {
  compositeOnly: { text: 'Animations using only transform and opacity run on the GPU for 60fps.', guideLink: '/guide/animations/parameters#performance' },
  layoutTrigger: { text: 'Animating width, height, or position causes expensive layout recalculation.', guideLink: '/guide/animations/parameters#performance' },
  paintTrigger: { text: 'Animating colors or shadows causes paint operations. Less costly than layout.', guideLink: '/guide/animations/parameters#performance' },
};

// ─── Class Names ────────────────────────────────────────────────

export const classNameTooltips: Record<string, TooltipContent> = {
  clientFirst: { text: 'Client-First methodology: descriptive, hierarchical class names like section_hero_heading.', guideLink: '/guide/figma-to-webflow/class-naming' },
  autoGenerated: { text: 'Class names like "Frame 42" are auto-generated by Figma and should be renamed.', guideLink: '/guide/figma-to-webflow/class-naming#auto-generated' },
};

// ─── Teams ──────────────────────────────────────────────────────

export const teamTooltips: Record<string, TooltipContent> = {
  teamRole: { text: 'Controls what actions a member can perform. Admins manage members, Members edit projects, Viewers read only.' },
  invitation: { text: 'Invitations expire after 7 days. The recipient must have a Forge account with the same email.' },
  teamSlug: { text: 'A unique URL identifier for your team. Use lowercase letters, numbers, and hyphens.' },
};

// ─── Community ──────────────────────────────────────────────────

export const communityTooltips: Record<string, TooltipContent> = {
  publish: { text: 'Share your template or preset with all Forge users. You can unpublish at any time.' },
  install: { text: 'Creates a copy in your personal library. Changes you make won\'t affect the original.' },
  featured: { text: 'Hand-picked by the Forge team for quality and usefulness.' },
};

// ─── Push to Webflow ────────────────────────────────────────────

export const pushTooltips: Record<string, TooltipContent> = {
  pushToWebflow: { text: 'Sends the structure directly to your Webflow site via the API.' },
  siteSelector: { text: 'Select which Webflow site to push to. Sites are loaded from your connected API token.' },
  pageSelector: { text: 'Choose the target page where elements will be created.' },
  masterScript: { text: 'Injects the generated animation runtime script into your site\'s custom code.' },
  scalingPush: { text: 'Adds the REM scaling CSS to your site header for responsive typography.' },
};
